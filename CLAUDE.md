...
Korean Law MCP Server v4.3.0 - 법제처 42개 API → 19개 통합 도구 (내부 95개) + 9개 시나리오 + 자연어 CLI + HTTP stateless + 판례 토큰 74% 감축 + **verify_citations (LLM 환각 방지)** + **impact_map (조문 영향 그래프)** + **time_travel (시점 diff)** + **action_plan (이럴 땐 이렇게, 5단계 안내)** + **cite_check (판례 생사 확인, v4.3)** + **applicable_law (행위시법 판단, v4.3)**
...
├── tool-registry.ts      # 95개 도구 등록, V3_EXPOSED 19개만 노출 (TOOL_COUNTS 파생값)
...
| `tool-registry.ts` | 95개 도구 정의, V3_EXPOSED 19개 노출 (TOOL_COUNTS 파생값) |
